...
@click.option('--output-dir', type=str, help="Directory to save the model and plots")
@click.option('--target-col', type=str, default='class', help="Name of the target/label column")
@click.option('--seed', type=int, default=522, help="Random seed")
def main(train_data, preprocessor, output_dir, target_col, seed):
    '''
    ...
    # 2. Fit and Get the Best Paramters of the  Model
    ...
    model_path = os.path.join(output_dir, "svc_pipeline.pickle")
    with open(model_path, 'wb') as f:
        pickle.dump(best_model, f)
    print(f"Model saved to {model_path}")

    # 4. Generate and Save Confusion Matrix
    # Note: We use plt.savefig instead of plt.show for script execution
    ...
    plot_path = os.path.join(output_dir, "svc_confusion_matrix.png")
